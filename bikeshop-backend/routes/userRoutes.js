...
router.get('/user/:id', isAuth, async (req, res) => {
    ...
    try {
        const connection = await mysql.getConnection();
        const [rows] = await connection.execute('SELECT id, lastName, firstName, email, role, imageUrl, FROM users WHERE id = ?', [id]);
        connection.release();
...
router.put('/user/:id', isAuth, async (req, res) => {
    ...
    try {
        ...
        await connection.execute('UPDATE users SET lastName = ?, firstName = ?, email = ?, WHERE id = ?', [lastName, firstName, email, id]);
        const [updatedUser] = await connection.execute(
            'SELECT id, lastName,firstName, email, role, imageUrl FROM users WHERE id = ?', 
            [id]
        );
